	...
	def visit(indent:Int, node : Node) : Unit = {
		...
		node match {
			case (Model(pkg:Package, pageflows:List[Pageflow])) =>
				store(node)
				visit(indent+1, pkg)
				for (pageflow <- pageflows) visit(indent+1, pageflow)
			case (Package(name:String)) =>
				store(name, node)
			case (Pageflow(name:String, actions:List[Action], states:List[State])) =>
				store(name, node)
				for (action <- actions) visit(indent+1, action)
				for (state <- states) visit(indent+1, state)
...
///////////////////////////////////////////////////////////////////////////////////////////
//                                                                                       //
// This generated class is meant to work together with Vaadin Pageflow extension, which  //
// can be found at:                                                                      //
//                                                                                       //
//   http://rgomes.svn.sourceforge.net/svnroot/rgomes/trunk/me.rgomes.vaadin_pageflow    //
//                                                                                       //
// This class and the aforementioned project are provided as it is, without any support, //
// warranty or suitability to any specific purpose by any means.                         //
//                                                                                       //
///////////////////////////////////////////////////////////////////////////////////////////
